...
import { drumKitList, DrumPlayer } from "./PlaySound";
...
const CreatePlayerContextProvider = (props) => {
  ...
  const handleSetMelodicTrack = (track, i, tone) => {
    ...
    if (isActiveTone) {
      const currentNote = currentTrack.beats[i].notes.find(
        (t) => t.tone === tone
      );
      ...
      if (durationNote > 1) {
        for (let j = 1; j < durationNote; j++) {
          ...
          const indexSameTone = nextNotesArray.findIndex(
            (t) => t.tone === tone
          );
          if (indexSameTone !== -1) {
            ...
            for (let k = 1; k < durationNext; k++) {
              ...
              const nextIndexSameTone = nextNotesArray.findIndex(
                (t) => t.tone === tone
              );
  ...
  const handleRestoreMusic = (id) => {
    axios
      .get(`https://api-melodiv.onrender.com/${id}`)
